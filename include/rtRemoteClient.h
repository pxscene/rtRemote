...
public:
  ...
  rtError sendSet(std::string const& objectId, char const* propertyName, rtValue const& value, rtValue& session);
  rtError sendGet(std::string const& objectId, uint32_t    propertyIdx,  rtValue& result);
  rtError sendGet(std::string const& objectId, char const* propertyName, rtValue& result, rtValue& session);
